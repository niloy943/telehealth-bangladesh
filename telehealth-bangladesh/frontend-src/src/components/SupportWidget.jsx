          ...
          >
            ...
            <div className="px-4 py-3.5 border-b border-medical-borderBg flex justify-between items-center bg-medical-darkBg/20">
              ...
              <button 
                ...
                className="text-medical-textMuted hover:text-white transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            {/* Tab switch buttons */}
            <div className="flex border-b border-medical-borderBg/50 text-[10px] font-bold uppercase tracking-wider bg-medical-darkBg/30">
              <button 
                onClick={() => setActiveTab('ai')}
                className={`flex-1 py-2 text-center transition-colors border-b-2 ${activeTab === 'ai' ? 'border-medical-teal text-medical-teal bg-medical-teal/5' : 'border-transparent text-medical-textMuted hover:text-white'}`}
              >
                AI Diagnostics
              </button>
              <button 
                onClick={() => setActiveTab('tickets')}
                className={`flex-1 py-2 text-center transition-colors border-b-2 ${activeTab === 'tickets' ? 'border-medical-teal text-medical-teal bg-medical-teal/5' : 'border-transparent text-medical-textMuted hover:text-white'}`}
              >
                File Ticket
              </button>
              <button 
                onClick={() => setActiveTab('hotlines')}
                className={`flex-1 py-2 text-center transition-colors border-b-2 ${activeTab === 'hotlines' ? 'border-medical-teal text-medical-teal bg-medical-teal/5' : 'border-transparent text-medical-textMuted hover:text-white'}`}
              >
                Hotlines
              </button>
            </div>
                ...
                <div className="flex-grow flex flex-col justify-between h-full">
                  <div className="flex-grow overflow-y-auto pr-1 space-y-2.5 max-h-[240px] flex flex-col">
                    {messages.map(msg => (
                      <div 
                        key={msg.id}
                        className={`p-2.5 rounded-xl text-[11px] max-w-[80%] leading-relaxed ${msg.sender === 'ai' ? 'bg-medical-darkBg border border-medical-borderBg self-start text-medical-textBody' : 'bg-medical-teal/10 border border-medical-teal/20 text-medical-teal self-end'}`}
                      >
                        {msg.text}
                      </div>
                    ))}
                  ...
                  <form onSubmit={handleSend} className="flex gap-2 border-t border-medical-borderBg/50 pt-3 mt-3">
                    <input 
                      ...
                      className="flex-grow bg-medical-darkBg border border-medical-borderBg rounded-xl px-3 py-2 text-[11px] outline-none text-white focus:border-medical-teal"
                    />
                ...
                <div className="flex-grow flex flex-col justify-between h-full">
                  {ticketSuccess ? (
                    <div className="my-auto text-center space-y-3 animate-fade">
                      ...
                      <p className="text-[10px] text-medical-textMuted">Our administrators will review and respond to your query shortly.</p>
                    </div>
                  ) : (
                    <form onSubmit={handleTicketSubmit} className="space-y-3">
                      <div>
                        <label className="block text-[10px] font-semibold text-medical-textMuted mb-1">Subject</label>
                        <input 
                          required
                          type="text" 
                          ...
                          className="w-full bg-medical-darkBg border border-medical-borderBg rounded-xl p-2.5 text-[11px] outline-none text-white focus:border-medical-teal"
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] font-semibold text-medical-textMuted mb-1">Detailed Inquiry</label>
                        <textarea 
                          required
                          rows="4" 
              ...
              {activeTab === 'hotlines' && (
                <div className="flex-grow space-y-3.5 animate-fade overflow-y-auto max-h-[280px] pr-1">
                  ...
                  <div className="space-y-2.5">
                    <div className="bg-medical-darkBg/60 border border-medical-borderBg p-3 rounded-xl flex items-center justify-between">
                      <div>
                        <h4 className="text-[11px] font-bold text-white">999 National Emergency</h4>
                        <p className="text-[9px] text-medical-textMuted">Ambulance, Police &amp; Fire Services</p>
                      </div>
                    ...
                    <div className="bg-medical-darkBg/60 border border-medical-borderBg p-3 rounded-xl flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[9px] text-medical-textMuted">Govt. Telehealth services (24/7)</p>
                      </div>
                    ...
                    <div className="bg-medical-darkBg/60 border border-medical-borderBg p-3 rounded-xl flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[9px] text-medical-textMuted">Government information &amp; social aid</p>
                      </div>
