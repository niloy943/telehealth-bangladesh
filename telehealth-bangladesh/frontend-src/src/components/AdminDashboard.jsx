    ...
    <div className="space-y-8 text-base animate-fade">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="glass-panel p-8 rounded-2xl space-y-3 spotlight-card tilt-card">
          ...
          <p className="text-sm text-slate-500 mt-1">Daphne websocket broker online</p>
        </div>
        ...
        <div className="glass-panel p-8 rounded-2xl space-y-3 spotlight-card tilt-card">
          ...
          <p className="text-sm text-slate-500 mt-1">Consent logs audited dynamically</p>
        </div>
        ...
        <div className="glass-panel p-8 rounded-2xl space-y-3 spotlight-card tilt-card">
          ...
          <p className="text-sm text-slate-500 mt-1">Seeded in sqlite database</p>
        </div>
        ...
        <div className="glass-panel p-8 rounded-2xl space-y-3 spotlight-card tilt-card">
          ...
          <p className="text-sm text-slate-500 mt-1">GDPR ledger records</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-panel p-8 rounded-2xl border border-white/5 space-y-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 border-b border-white/5 pb-3">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-0.5">Real-time HIPAA &amp; GDPR compliance stream</p>
            </div>
            <button onClick={loadAdminData} className="text-sm text-slate-400 hover:text-white flex items-center gap-1.5 font-bold">
          ...
          <div className="flex flex-col md:flex-row gap-3">
            <div className="relative flex-grow">
              <input 
                ...
                className="w-full bg-slate-900 border border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-white outline-none focus:border-medical-teal placeholder-slate-500 text-base" 
              />
            ...
            <div className="flex gap-1.5">
              {['all', 'decrypt', 'kyc', 'auth'].map(f => (
                <button
                  ...
                  className={`px-4.5 py-2 rounded-xl font-bold uppercase text-xs transition-all border ${auditFilter === f ? 'bg-medical-teal/10 border-medical-teal/30 text-medical-teal' : 'bg-slate-900 border-white/5 text-slate-400 hover:text-white'}`}
                >
                  {f}
                </button>
              ))}
            ...
            <table className="w-full text-left text-sm text-slate-400">
              ...
              <tbody>
                {logs.error ? (
                  ...
                  <tr>
                    <td colSpan="6" className="py-4 text-center text-slate-500">No logs matching selected filter.</td>
                  </tr>
                ) : (
                  filteredLogs.map(log => (
                    <tr key={log.id} className="border-b border-white/5 hover:bg-white/2">
                      ...
                      <td className="px-2 capitalize text-xs text-slate-500">{log.user_details?.role || "System"}</td>
                      <td className="px-2">
                      ...
                      <td className="px-2 font-mono text-xs text-slate-500">{log.ip_address || "127.0.0.1"}</td>
                      <td className="px-2 max-w-xs truncate text-xs text-slate-300" title={log.details}>{log.details}</td>
                    </tr>
                  ))
                )}
              </tbody>
        ...
        <div className="space-y-6">
          ...
          <div className="glass-panel p-8 rounded-2xl border border-white/5 space-y-6 spotlight-card tilt-card">
            <h3 className="text-xl font-bold text-slate-200 border-b border-white/5 pb-2 uppercase tracking-wider">System Live Diagnostics</h3>
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                ...
                <div className="w-full h-16 bg-slate-950/60 rounded-lg overflow-hidden border border-white/5 relative">
          ...
          <div className="glass-panel p-8 rounded-2xl border border-white/5 space-y-6 spotlight-card tilt-card">
            <h3 className="text-xl font-bold text-slate-200 border-b border-white/5 pb-2 uppercase tracking-wider">Security Activity Matrix</h3>
            <div className="grid grid-cols-2 gap-4 text-xs">
              <div className="bg-slate-950/40 p-4 rounded-xl border border-white/5 flex flex-col justify-between">
                <span className="text-slate-500 font-bold uppercase tracking-wider">Brute Locks</span>
              ...
              <div className="bg-slate-950/40 p-4 rounded-xl border border-white/5 flex flex-col justify-between">
                <span className="text-slate-500 font-bold uppercase tracking-wider">E2EE Tunnels</span>
                <span className="text-2xl font-extrabold text-medical-teal mt-1.5">5 <span className="text-xs text-slate-400 font-mono font-normal">Active</span></span>
              </div>
              <div className="bg-slate-950/40 p-4 rounded-xl border border-white/5 flex flex-col justify-between">
                <span className="text-slate-500 font-bold uppercase tracking-wider">Failed Logins</span>
                <span className="text-2xl font-extrabold text-medical-rose mt-1.5">2 <span className="text-xs text-slate-400 font-mono font-normal">Ip rate-lim</span></span>
              </div>
              <div className="bg-slate-950/40 p-4 rounded-xl border border-white/5 flex flex-col justify-between">
                <span className="text-slate-500 font-bold uppercase tracking-wider">KYC Compliance</span>
                <span className="text-2xl font-extrabold text-medical-indigo mt-1.5">94% <span className="text-xs text-slate-400 font-mono font-normal">Audited</span></span>
              </div>
            </div>
          </div>
          ...
          <div className="glass-panel p-8 rounded-2xl border border-white/5 space-y-6">
            <h3 className="text-xl font-bold text-slate-200 border-b border-white/5 pb-2 uppercase tracking-wider">{t('kycReviewCenter')}</h3>
            
            <div className="space-y-4 max-h-[260px] overflow-y-auto pr-1">
              {doctors.length === 0 ? (
                <p className="text-sm text-slate-500 py-8 text-center">No pending licenses for review.</p>
              ) : (
                doctors.map(d => (
                  <div key={d.id} className="bg-slate-950/40 p-4 rounded-xl border border-white/5 space-y-3">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-1">BMDC: {d.user.bmdc_reg || "BMDC/A-04938"}</p>
                      </div>
                    ...
                    <div className="flex gap-2 justify-end pt-1">
                      ...
                      <button onClick={() => handleRejectDoctor(d.id, d.user.bmdc_reg || "BMDC/A-04938")} className="bg-slate-900 text-slate-400 px-3 py-1.5 rounded-lg text-xs font-bold border border-white/10 hover:text-white transition-all flex items-center gap-1.5">
        ...
        <div className="overflow-x-auto max-h-[300px] pr-1">
          <table className="w-full text-left text-sm text-slate-400">
            ...
            <tbody>
              {orders.length === 0 ? (
                <tr>
                  <td colSpan="6" className="text-center py-6 text-slate-500">No shipments found in log database.</td>
                </tr>
              ) : (
                orders.map(order => (
                    ...
                    <td className="py-2">
                      <div className="flex gap-2">
                        {order.status === 'pending' && (
                          <button onClick={() => handleUpdateOrderStatus(order.id, 'packing')} className="bg-slate-900 border border-white/10 hover:bg-slate-800 text-slate-300 px-3 py-1.5 rounded-lg text-xs font-bold">
                            Pack
                          </button>
                        )}
