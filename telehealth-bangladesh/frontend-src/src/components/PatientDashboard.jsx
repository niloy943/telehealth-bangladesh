  ...
  element.innerHTML = `
    <div style="border: 2px solid #14b8a6; padding: 25px; border-radius: 12px; position: relative;">
      <div style="display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 3px solid #14b8a6; padding-bottom: 15px; margin-bottom: 20px;">
        <div>
          <h1 style="margin: 0; color: #14b8a6; font-size: 28px; font-weight: 800;">SwasthoNirapod</h1>
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; border-top: 1px dashed #cbd5e1; padding-top: 15px; font-size: 11px; color: #64748b;">
        <div>
          <p style="margin: 2px 0;">This document is issued securely via <strong>SwasthoNirapod</strong>.</p>
  ...
  return (
    <div className="space-y-8 text-base">
      ...
      <div className="flex gap-3 border-b border-medical-borderBg pb-4 overflow-x-auto">
        <button onClick={() => { setSubView("overview"); if (onTabChange) onTabChange("dashboard"); }} className={`px-5 py-3 rounded-xl font-bold transition-all ${subView === 'overview' ? 'bg-medical-teal text-medical-darkBg shadow-md shadow-medical-teal/15' : 'text-slate-400 hover:text-white bg-slate-900/40'}`}>
          {t('vitalsOverview')}
        </button>
        <button onClick={() => { setSubView("booking"); if (onTabChange) onTabChange("booking"); }} className={`px-5 py-3 rounded-xl font-bold transition-all ${subView === 'booking' ? 'bg-medical-teal text-medical-darkBg shadow-md shadow-medical-teal/15' : 'text-slate-400 hover:text-white bg-slate-900/40'}`}>
          {t('bookNewAppt')}
        </button>
        <button onClick={() => { setSubView("records"); if (onTabChange) onTabChange("records"); }} className={`px-5 py-3 rounded-xl font-bold transition-all ${subView === 'records' ? 'bg-medical-teal text-medical-darkBg shadow-md shadow-medical-teal/15' : 'text-slate-400 hover:text-white bg-slate-900/40'}`}>
          {t('encryptedRecords')}
        </button>
        <button onClick={() => { setSubView("consent"); if (onTabChange) onTabChange("consent"); }} className={`px-5 py-3 rounded-xl font-bold transition-all ${subView === 'consent' ? 'bg-medical-teal text-medical-darkBg shadow-md shadow-medical-teal/15' : 'text-slate-400 hover:text-white bg-slate-900/40'}`}>
          {t('consentManager')}
        </button>
        <button onClick={() => { setSubView("pharmacy"); if (onTabChange) onTabChange("pharmacy"); }} className={`px-5 py-3 rounded-xl font-bold transition-all ${subView === 'pharmacy' ? 'bg-medical-teal text-medical-darkBg shadow-md shadow-medical-teal/15' : 'text-slate-400 hover:text-white bg-slate-900/40'}`}>
          {t('medicineDelivery')}
        </button>
        <button onClick={() => { setSubView("ai"); if (onTabChange) onTabChange("ai"); }} className={`px-5 py-3 rounded-xl font-bold transition-all ${subView === 'ai' ? 'bg-medical-teal text-medical-darkBg shadow-md shadow-medical-teal/15' : 'text-slate-400 hover:text-white bg-slate-900/40'}`}>
          {t('aiAssistant')}
        </button>
      </div>
        ...
        <div className="space-y-6 animate-fade relative overflow-hidden">
          ...
          <div className="glass-panel p-8 rounded-2xl flex flex-col md:flex-row justify-between items-center gap-6 bg-gradient-to-r from-medical-indigo/10 via-medical-cardBg to-medical-teal/10 border-medical-indigo/20 relative z-10 spotlight-card tilt-card">
            <div>
              ...
              <p className="text-base text-slate-400 mt-1">Your secure telemedicine workspace is fully encrypted and clinical keys are active.</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
            ...
            <div className="glass-panel p-8 rounded-2xl space-y-4 relative overflow-hidden group spotlight-card tilt-card">
              <div className="flex justify-between items-center gap-4">
                <div className="space-y-2 flex-grow">
                  ...
                  <p className="text-xs text-slate-500 mt-1">Steady rhythm • Synced 3m ago</p>
                </div>
                ...
                <div className="relative w-16 h-16 shrink-0 flex items-center justify-center">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center text-sm font-extrabold text-slate-300">
              ...
              <div className="flex gap-2 border-t border-white/5 pt-3">
                <button onClick={() => setVitals(v => ({ ...v, heartRate: Math.max(50, v.heartRate - 2) }))} className="magnetic-target bg-slate-900 border border-white/10 px-3 py-1.5 rounded-lg text-sm text-slate-400 hover:text-white transition-colors">- 2</button>
                <button onClick={() => setVitals(v => ({ ...v, heartRate: Math.min(150, v.heartRate + 2) }))} className="magnetic-target bg-slate-900 border border-white/10 px-3 py-1.5 rounded-lg text-sm text-slate-400 hover:text-white transition-colors">+ 2</button>
              </div>
            </div>
            ...
            <div className="glass-panel p-8 rounded-2xl space-y-4 relative overflow-hidden group spotlight-card tilt-card">
              <div className="flex justify-between items-center gap-4">
                <div className="space-y-2 flex-grow">
                  ...
                  <h3 className="text-3xl font-extrabold text-white flex items-baseline gap-1">
                    ...
                    <span className="text-sm text-slate-500 font-semibold">/ 2.5 L</span>
                  </h3>
                  <p className="text-xs text-slate-500 mt-1">Daily hydration target status</p>
                </div>
              ...
              <div className="flex gap-2 border-t border-white/5 pt-3">
                <button onClick={() => setVitals(v => ({ ...v, waterIntake: Math.max(0, v.waterIntake - 0.25) }))} className="magnetic-target bg-slate-900 border border-white/10 px-3 py-1.5 rounded-lg text-sm text-slate-400 hover:text-white transition-colors">- 250ml</button>
                <button onClick={() => setVitals(v => ({ ...v, waterIntake: Math.min(5, v.waterIntake + 0.25) }))} className="magnetic-target bg-slate-900 border border-white/10 px-3 py-1.5 rounded-lg text-sm text-slate-400 hover:text-white transition-colors">+ 250ml</button>
              </div>
            </div>
            ...
            <div className="glass-panel p-8 rounded-2xl space-y-4 relative overflow-hidden group spotlight-card tilt-card">
              <div className="flex justify-between items-center gap-4">
                <div className="space-y-2 flex-grow">
                  ...
                  <h3 className="text-3xl font-extrabold text-white flex items-baseline gap-1">
                    ...
                    <span className="text-sm text-slate-500 font-semibold">hrs</span>
                  </h3>
                  <p className="text-xs text-slate-500 mt-1">Restful sleep quality • 92% efficiency</p>
                </div>
              ...
              <div className="flex gap-2 border-t border-white/5 pt-3">
                <button onClick={() => setVitals(v => ({ ...v, sleepHrs: Math.max(0, v.sleepHrs - 0.5) }))} className="magnetic-target bg-slate-900 border border-white/10 px-3 py-1.5 rounded-lg text-sm text-slate-400 hover:text-white transition-colors">- 0.5h</button>
                <button onClick={() => setVitals(v => ({ ...v, sleepHrs: Math.min(24, v.sleepHrs + 0.5) }))} className="magnetic-target bg-slate-900 border border-white/10 px-3 py-1.5 rounded-lg text-sm text-slate-400 hover:text-white transition-colors">+ 0.5h</button>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 relative z-10">
            ...
            <div className="glass-panel p-8 rounded-2xl space-y-6 spotlight-card tilt-card">
              <div className="flex justify-between items-center border-b border-slate-200 dark:border-white/5 pb-3">
                <h3 className="text-xl font-bold text-slate-800 dark:text-white uppercase tracking-wider">Health Stats Trends (Daily Blood Sugar Logs)</h3>
                <span className="text-sm text-slate-500 font-medium">Symmetric encrypted key active</span>
              </div>
            ...
            <div className="glass-panel p-8 rounded-2xl space-y-6 spotlight-card tilt-card">
              <div className="flex justify-between items-center border-b border-slate-200 dark:border-white/5 pb-3">
                <h3 className="text-xl font-bold text-slate-800 dark:text-white uppercase tracking-wider">Interactive Health Timeline</h3>
                <span className="text-sm text-slate-500 font-medium">Real-time ledger events</span>
              </div>
              ...
              <div className="relative pl-6 space-y-6 before:content-[''] before:absolute before:left-2 before:top-2 before:bottom-2 before:w-[2px] before:bg-gradient-to-b before:from-medical-teal before:via-medical-indigo before:to-transparent">
                ...
                <div className="relative group">
                  <span className="absolute -left-6 top-1.5 w-4.5 h-4.5 rounded-full bg-slate-950 border-2 border-medical-teal flex items-center justify-center group-hover:scale-125 transition-transform z-10">
                    <span className="w-2 h-2 rounded-full bg-medical-teal security-indicator animate-ping" />
                  </span>
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-2">
                      <span className="text-base font-extrabold text-slate-850 dark:text-white">Sugar Level Synced</span>
                      <span className="text-xs bg-medical-teal/15 text-medical-teal px-2 py-0.5 rounded font-mono font-bold">5.8 mmol/L</span>
                    </div>
                    <p className="text-sm text-slate-650 dark:text-slate-400 leading-normal">Decrypted metrics logs recorded via local Bluetooth health monitor sync.</p>
                    <span className="text-xs text-slate-500 block font-mono font-semibold">Today, 02:40 PM</span>
                  </div>
                </div>

                {/* Timeline Item 2 */}
                <div className="relative group">
                  <span className="absolute -left-6 top-1.5 w-4.5 h-4.5 rounded-full bg-slate-950 border-2 border-medical-indigo flex items-center justify-center group-hover:scale-125 transition-transform z-10">
                    <span className="w-2 h-2 rounded-full bg-medical-indigo" />
                  </span>
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-2">
                      <span className="text-base font-extrabold text-slate-850 dark:text-white">Encryption Keys Rotated</span>
                      <span className="text-xs bg-medical-indigo/15 text-medical-indigo px-2 py-0.5 rounded font-mono font-bold">256-bit AES</span>
                    </div>
                    <p className="text-sm text-slate-650 dark:text-slate-400 leading-normal">Security vault rotated symmetric tokens for the active consultation session with Dr. Jenkins.</p>
                    <span className="text-xs text-slate-500 block font-mono font-semibold">Yesterday, 10:15 AM</span>
                  </div>
                </div>

                {/* Timeline Item 3 */}
                <div className="relative group">
                  <span className="absolute -left-6 top-1.5 w-4.5 h-4.5 rounded-full bg-slate-950 border-2 border-medical-emerald flex items-center justify-center group-hover:scale-125 transition-transform z-10">
                    <span className="w-2 h-2 rounded-full bg-medical-emerald" />
                  </span>
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-2">
                      <span className="text-base font-extrabold text-slate-850 dark:text-white">Medical Consent Granted</span>
                    ...
                    <p className="text-sm text-slate-650 dark:text-slate-400 leading-normal">Authorized medical records access permission set to active for Dr. Sarah Jenkins (Cardiology).</p>
                    <span className="text-xs text-slate-500 block font-mono font-semibold">05 Jun 2026, 03:00 PM</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="glass-panel p-8 rounded-2xl space-y-6">
            <h3 className="text-xl font-bold text-slate-850 dark:text-slate-200 border-b border-slate-200 dark:border-white/5 pb-3 uppercase tracking-wider">E-Prescription Records</h3>
            {prescriptions.length === 0 ? (
              <p className="text-base text-slate-500 py-6 text-center">No prescriptions found.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {prescriptions.map(p => (
                  <div key={p.id} className="bg-slate-100 dark:bg-slate-950/40 p-6 border border-slate-200 dark:border-white/5 rounded-xl space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xs text-slate-500 font-mono">{new Date(p.created_at).toLocaleDateString()}</span>
                    </div>
                    
                    <div className="text-base text-slate-600 dark:text-slate-400 space-y-2 mt-1">
                      <p><strong className="text-slate-700 dark:text-slate-300">Symptoms:</strong> {p.symptoms}</p>
                      <p><strong className="text-slate-700 dark:text-slate-300">Diagnosis:</strong> {p.diagnosis}</p>
                      <div className="mt-2">
                        <strong className="text-slate-700 dark:text-slate-300">Medications (Rx):</strong>
                        <ul className="list-disc pl-5 mt-1 text-sm space-y-1">
                          {(() => {
                            try {
                              const meds = JSON.parse(p.medicines);
                              return Array.isArray(meds) ? meds.map((m, idx) => (
                                <li key={idx} className="text-slate-650 dark:text-slate-400">
                                  <span className="font-semibold text-slate-800 dark:text-slate-200">{m.name}</span> - {m.dosage} ({m.timing})
                                </li>
                              )) : <li>{p.medicines}</li>;
                            } catch (e) {
          ...
          <div className="glass-panel p-8 rounded-2xl">
            <div className="flex justify-between items-center mb-6 border-b border-slate-200 dark:border-white/5 pb-3">
              <h3 className="text-xl font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider">{t('upcomingAppt')}</h3>
                ...
                <table className="w-full text-left text-xs text-slate-400">
                  ...
                  <tbody>
                    {appointments.map(appt => (
                      <tr key={appt.id} className="border-b border-white/5 hover:bg-white/2">
      ...
      {subView === 'booking' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
              ...
              <div className="flex gap-2 overflow-x-auto w-full md:w-auto">
                <button onClick={() => setSpecialtyFilter("all")} className={`px-3.5 py-1.5 rounded-lg text-xs font-bold uppercase transition-all ${specialtyFilter === 'all' ? 'bg-medical-teal text-medical-darkBg' : 'bg-slate-900 border border-white/5 text-slate-400'}`}>All</button>
                <button onClick={() => setSpecialtyFilter("Cardiology")} className={`px-3.5 py-1.5 rounded-lg text-xs font-bold uppercase transition-all ${specialtyFilter === 'Cardiology' ? 'bg-medical-teal text-medical-darkBg' : 'bg-slate-900 border border-white/5 text-slate-400'}`}>Cardiology</button>
                <button onClick={() => setSpecialtyFilter("Pediatrics")} className={`px-3.5 py-1.5 rounded-lg text-xs font-bold uppercase transition-all ${specialtyFilter === 'Pediatrics' ? 'bg-medical-teal text-medical-darkBg' : 'bg-slate-900 border border-white/5 text-slate-400'}`}>Pediatrics</button>
                <button onClick={() => setSpecialtyFilter("General Practice")} className={`px-3.5 py-1.5 rounded-lg text-xs font-bold uppercase transition-all ${specialtyFilter === 'General Practice' ? 'bg-medical-teal text-medical-darkBg' : 'bg-slate-900 border border-white/5 text-slate-400'}`}>General</button>
              </div>
            </div>

            {/* Search Bar */}
            <div className="relative">
              <input type="text" placeholder={t('searchDocs')} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-slate-900 border border-white/10 focus:border-medical-teal rounded-xl py-3 pl-11 pr-4 outline-none text-white placeholder-slate-500 text-base" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[550px] overflow-y-auto pr-1">
              {filteredDoctors.map(doc => (
                <div key={doc.id} onClick={() => setSelectedDocId(doc.user.id)} className={`glass-panel p-6 rounded-xl cursor-pointer transition-all border ${selectedDocId === doc.user.id ? 'border-medical-teal bg-medical-teal/5 shadow-md shadow-medical-teal/5' : 'border-white/5'}`}>
                  <div className="flex gap-4 items-start">
                    <div className="w-12 h-12 rounded-full bg-slate-800 flex items-center justify-center font-bold text-medical-teal text-base">
                    ...
                    <div className="flex-grow">
                      ...
                      <p className="text-xs text-slate-500 mt-1">{doc.hospital || 'Private Clinic'}</p>
                    </div>
          ...
          <div className="glass-panel p-8 rounded-2xl border border-white/5 h-fit">
            <h3 className="text-xl font-bold text-slate-200 mb-6 uppercase tracking-wider">{t('scheduleDetails')}</h3>
            
            {selectedDocId ? (
              <form onSubmit={handleBookAppointment} className="space-y-6">
                <div>
                  <label className="block text-sm font-bold text-slate-400 mb-1.5">{t('consultationMode')}</label>
                  <select value={apptForm.consultation_type} onChange={e => setApptForm({...apptForm, consultation_type: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white outline-none focus:border-medical-teal text-base">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-bold text-slate-400 mb-1.5">{t('selectDate')}</label>
                    <input required type="date" value={apptForm.date} min="2026-06-06" onChange={e => setApptForm({...apptForm, date: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white outline-none focus:border-medical-teal text-base" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-400 mb-1.5">{t('selectTime')}</label>
                    <select value={apptForm.time} onChange={e => setApptForm({...apptForm, time: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white outline-none focus:border-medical-teal text-base">
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-400 mb-1.5">{t('symptomsReason')}</label>
                  <textarea required rows="3" placeholder="Briefly describe your symptoms..." value={apptForm.reason} onChange={e => setApptForm({...apptForm, reason: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white outline-none focus:border-medical-teal text-base"></textarea>
                </div>
      ...
      {subView === 'records' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade">
          {/* Uploader (Left) */}
          <div className="glass-panel p-8 rounded-2xl border border-white/5 h-fit space-y-6">
            ...
            <div 
              ...
              className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${recordDragOver ? 'border-medical-teal bg-medical-teal/5' : 'border-white/10 hover:border-medical-teal/30 bg-slate-900/30'}`}
            >
              {recordSuccess ? (
                ...
                <div className="flex flex-col items-center justify-center gap-2">
                  ...
                  <span className="font-bold text-slate-300 text-base">Drag &amp; Drop scans here</span>
                  <span className="text-xs text-slate-500 uppercase font-semibold">Or click upload PDF (Max 5MB)</span>
                </div>
              )}
            </div>

            <div className="border-t border-slate-100 dark:border-white/5 pt-5">
              <p className="text-xs text-slate-500 uppercase font-bold text-center mb-4">Or fill index parameters</p>
              
              <form onSubmit={handleCreateRecord} className="space-y-6">
                <div>
                  <label className="block text-sm font-bold text-slate-400 mb-1.5">{t('recordCategory')}</label>
                  <select value={recordForm.record_type} onChange={e => setRecordForm({...recordForm, record_type: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white outline-none focus:border-medical-teal text-base">
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-400 mb-1.5">{t('recordData')}</label>
                  <textarea required rows="3" placeholder="Enter clinical vitals, values, or notes..." value={recordForm.content} onChange={e => setRecordForm({...recordForm, content: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white outline-none focus:border-medical-teal text-base"></textarea>
                </div>
            ...
            <div className="border-t border-slate-100 dark:border-white/5 pt-5 space-y-4">
              <p className="text-xs text-slate-500 uppercase font-bold text-center">Legacy Medical Graphic Upload</p>
              <div className="space-y-3 bg-slate-950/20 p-4 rounded-xl border border-white/5">
                <input required type="text" placeholder="Scan Label (e.g. My ECG Graph)" value={legacyImageName} onChange={e => setLegacyImageName(e.target.value)} className="w-full bg-slate-900 border border-white/10 rounded-lg p-2.5 text-xs text-white outline-none focus:border-medical-teal" />
                
                <label className="block bg-slate-900 border border-white/10 hover:border-medical-teal/30 cursor-pointer rounded-lg p-3 text-center text-xs text-slate-400 transition-colors">
          ...
          <div className="lg:col-span-2 glass-panel p-8 rounded-2xl border border-white/5 space-y-6">
            <h3 className="text-xl font-bold text-slate-200 border-b border-slate-100 dark:border-white/5 pb-3 uppercase tracking-wider">{t('clinicalIndex')}</h3>

            {/* Vision Analysed Legacy Summaries */}
            <div className="space-y-3 border-b border-slate-100 dark:border-white/5 pb-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[220px] overflow-y-auto pr-1">
                {legacyImages.length === 0 ? (
                  <p className="text-slate-500 text-xs py-2">No legacy scans analyzed yet. Upload an image on the left to extract chart metrics.</p>
                ) : (
                  legacyImages.map(img => (
                    <div key={img.id} className="bg-slate-900/60 p-3 rounded-lg border border-white/5 space-y-1">
                      <div className="flex justify-between items-center text-[10px] font-bold text-slate-300">
                         <span>{img.image_name}</span>
                         <span className="text-[8px] text-slate-500">{new Date(img.created_at).toLocaleDateString()}</span>
                      </div>
                      <p className="font-mono text-[9px] text-slate-400 bg-black/40 p-2 rounded border border-white/5 whitespace-pre-wrap">
            ...
            <div className="space-y-6 max-h-[350px] overflow-y-auto">
              {records.length === 0 ? (
                <p className="text-slate-500 text-center py-12 text-base">{t('emptyRecords')}</p>
              ) : (
                records.map(rec => (
                  <div key={rec.id} className="bg-slate-100 dark:bg-slate-950/40 border border-slate-250 dark:border-white/5 rounded-xl p-5 flex gap-4 items-start">
                    ...
                    <div className="flex-grow space-y-2">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-xs text-slate-500 font-mono">{new Date(rec.updated_at).toLocaleDateString()}</span>
                      </div>
                      <p className="font-mono text-slate-700 dark:text-slate-300 leading-relaxed bg-slate-200/50 dark:bg-black/40 p-4 rounded-lg border border-slate-300 dark:border-white/5 text-sm">
      ...
      {subView === 'consent' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade">
          ...
          <div className="glass-panel p-8 rounded-2xl border border-white/5 h-fit">
            ...
            <p className="text-sm text-slate-400 mb-6 leading-relaxed">{t('delegateDesc')}</p>
            
            <form onSubmit={handleGrantConsent} className="space-y-6">
              <div>
                <label className="block text-sm font-bold text-slate-400 mb-1.5">{t('selectPhysician')}</label>
                <select required value={consentForm.doctorId} onChange={e => setConsentForm({...consentForm, doctorId: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white outline-none focus:border-medical-teal text-base">
              ...
              <div>
                <label className="block text-sm font-bold text-slate-400 mb-1.5">{t('validityDuration')}</label>
                <select value={consentForm.hours} onChange={e => setConsentForm({...consentForm, hours: parseInt(e.target.value)})} className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white outline-none focus:border-medical-teal text-base">
          ...
          {/* Consents List (Right) */}
          <div className="lg:col-span-2 glass-panel p-8 rounded-2xl border border-white/5 space-y-6">
            <h3 className="text-xl font-bold text-slate-200 border-b border-slate-100 dark:border-white/5 pb-3 uppercase tracking-wider">{t('activeTokens')}</h3>
            
            <div className="space-y-4 max-h-[550px] overflow-y-auto pr-1">
              {consents.length === 0 ? (
                <p className="text-slate-500 text-center py-12 text-base">{t('emptyConsents')}</p>
              ) : (
                consents.map(con => (
                  <div key={con.id} className="bg-slate-100 dark:bg-slate-950/40 border border-slate-250 dark:border-white/5 rounded-xl p-5 flex justify-between items-center text-base">
                    <div>
                      <h4 className="font-bold text-white text-base">Dr. {con.doctor_details.first_name} {con.doctor_details.last_name}</h4>
                      <p className="text-xs text-slate-400 mt-1.5 flex items-center gap-1.5"><Clock className="w-4 h-4 text-medical-teal" /> Granted: {new Date(con.created_at).toLocaleString()}</p>
                      <p className="text-xs text-slate-400 mt-1 flex items-center gap-1.5"><Clock className="w-4 h-4 text-medical-rose" /> Expires: {new Date(con.expires_at).toLocaleString()}</p>
                    </div>
                    <div>
                      {con.granted ? (
                        ...
                        <span className="text-xs bg-slate-900 border border-white/5 px-3 py-1.5 rounded-lg text-slate-500 font-bold uppercase tracking-wider">{t('revoked')}</span>
                      )}
                    </div>
                  </div>
                ))
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade">
          ...
          <div className="glass-panel p-8 rounded-2xl border border-white/5 h-fit">
            ...
            <form onSubmit={handlePlaceOrder} className="space-y-6">
              <div>
                <label className="block text-sm font-bold text-slate-400 mb-1.5">Prescription Reference</label>
                <select value={selectedPrescId} onChange={e => setSelectedPrescId(e.target.value)} className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white outline-none focus:border-medical-teal text-base">
              ...
              <div>
                <label className="block text-sm font-bold text-slate-400 mb-1.5">{t('deliveryAddress')}</label>
                <textarea required rows="3" placeholder="Enter delivery address in Bangladesh" value={deliveryAddress} onChange={e => setDeliveryAddress(e.target.value)} className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white outline-none focus:border-medical-teal text-base"></textarea>
              </div>
          ...
          {/* Orders log (Right) */}
          <div className="lg:col-span-2 glass-panel p-8 rounded-2xl border border-white/5 space-y-6">
            <h3 className="text-xl font-bold text-slate-200 border-b border-slate-100 dark:border-white/5 pb-3 uppercase tracking-wider">{t('courierLogs')}</h3>
            
            <div className="space-y-4 max-h-[550px] overflow-y-auto pr-1">
              {orders.length === 0 ? (
                <p className="text-slate-500 text-center py-12 text-base">No orders placed in history.</p>
              ) : (
                orders.map(order => (
                  <div key={order.id} className="bg-slate-100 dark:bg-slate-950/40 border border-slate-250 dark:border-white/5 rounded-xl p-5 flex justify-between items-center text-base">
                    <div>
                      ...
                      <p className="text-sm text-slate-800 dark:text-slate-300 mt-2 font-bold">Estimated Cost: {order.total_price} BDT</p>
                    </div>
      ...
      {subView === 'ai' && (
        <div className="glass-panel p-8 rounded-2xl border border-white/5 space-y-6 max-w-2xl mx-auto animate-fade">
          <div className="flex items-center gap-3 border-b border-slate-100 dark:border-white/5 pb-4">
            <div className="bg-medical-teal/15 p-2 rounded-xl text-medical-teal">
              <Bot className="w-6 h-6" />
            </div>
            <div>
              <h3 className="text-base font-bold text-slate-800 dark:text-slate-200">{t('aiHeader')}</h3>
              <p className="text-xs text-slate-500">{t('aiDisclaimer')}</p>
            </div>
          </div>
          ...
          <div className="h-96 overflow-y-auto pr-1 space-y-4 flex flex-col">
            {aiMessages.map((msg, i) => (
              <div key={i} className={`p-4 rounded-xl max-w-[80%] leading-relaxed text-base ${msg.sender === 'ai' ? 'bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-white/5 self-start text-slate-750 dark:text-slate-350 font-medium' : 'bg-medical-teal/10 border border-medical-teal/15 text-teal-650 dark:text-medical-teal self-end font-medium'}`}>
                {msg.text}
              </div>
            ))}
          </div>
          ...
          {/* Message input */}
          <form onSubmit={handleSendAiMessage} className="flex gap-3 border-t border-slate-100 dark:border-white/5 pt-4">
            <input type="text" value={aiInput} onChange={e => setAiInput(e.target.value)} placeholder={t('aiPlaceholder')} className="flex-grow bg-slate-900 border border-white/10 focus:border-medical-teal rounded-xl px-4 py-3 outline-none text-white text-base" />
