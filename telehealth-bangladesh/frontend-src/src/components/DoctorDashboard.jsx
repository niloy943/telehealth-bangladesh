    ...
    <div className="space-y-8 text-base">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="glass-panel p-8 rounded-2xl space-y-3 spotlight-card tilt-card">
          ...
          <div className="flex items-center gap-3 justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">Pending / approved load</p>
            </div>
        ...
        <div className="glass-panel p-8 rounded-2xl space-y-3 spotlight-card tilt-card">
          ...
          <p className="text-xs text-slate-500 mt-1">{completedCount} consultation sessions</p>
        </div>
        ...
        <div className="glass-panel p-8 rounded-2xl space-y-3 spotlight-card tilt-card">
          ...
          <p className="text-xs text-slate-500 uppercase tracking-widest font-bold mt-1">Status: verified</p>
        </div>
        ...
        <div className="glass-panel p-8 rounded-2xl space-y-3 spotlight-card tilt-card">
          ...
          <button onClick={handleRegenKeys} className="magnetic-target text-xs text-slate-400 hover:text-medical-teal hover:underline flex items-center gap-1 font-semibold mt-1">
      ...
      <div className="glass-panel p-8 rounded-2xl border border-white/5 bg-gradient-to-r from-medical-teal/10 via-medical-cardBg to-medical-indigo/10 flex flex-col md:flex-row justify-between items-center gap-6 spotlight-card tilt-card">
        <div>
          <h4 className="text-xl font-bold text-white flex items-center gap-2">
            <span className={`status-light ${onlineStatus ? 'online' : 'offline'}`}></span>
            <span>{t('availabilityStatus')}</span>
          </h4>
          <p className="text-sm text-slate-400 mt-1.5">{t('availabilityDesc')}</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="glass-panel p-8 rounded-2xl space-y-6 spotlight-card tilt-card">
          <div className="flex justify-between items-center border-b border-slate-200 dark:border-white/5 pb-3">
            <h3 className="text-xl font-bold text-slate-800 dark:text-white uppercase tracking-wider">Monthly Consultation Revenue</h3>
          ...
          <div className="h-36 flex items-end justify-between pt-6 px-4">
            <div className="w-10 bg-slate-200 dark:bg-slate-800 rounded-t-lg h-[20%] text-center relative group">
            ...
            <div className="w-10 bg-slate-200 dark:bg-slate-800 rounded-t-lg h-[40%] text-center relative group">
            ...
            <div className="w-10 bg-slate-200 dark:bg-slate-800 rounded-t-lg h-[30%] text-center relative group">
            ...
            <div className="w-10 bg-slate-200 dark:bg-slate-800 rounded-t-lg h-[70%] text-center relative group">
        ...
        <div className="glass-panel p-8 rounded-2xl space-y-6 spotlight-card tilt-card">
          <div className="flex justify-between items-center border-b border-slate-200 dark:border-white/5 pb-3">
            <h3 className="text-xl font-bold text-slate-800 dark:text-white uppercase tracking-wider">Weekly Booking Density Heatmap</h3>
            <div className="flex items-center gap-2 text-xs text-slate-500 font-bold uppercase">
              <span>Low</span>
              <span className="w-3.5 h-3.5 bg-slate-900 rounded-sm" />
              <span className="w-3.5 h-3.5 bg-teal-900/60 rounded-sm" />
              <span className="w-3.5 h-3.5 bg-teal-500/50 rounded-sm" />
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-7 gap-2 text-center text-xs text-slate-500 font-mono font-bold">
              <span>MON</span>
              <span>TUE</span>
              <span>WED</span>
              <span>THU</span>
              ...
              {[
                ...
                return (
                  <div key={rIdx} className="flex gap-3 items-center">
                    <span className="w-16 text-right text-xs text-slate-500 font-mono shrink-0">{times[rIdx]}</span>
                    <div className="grid grid-cols-7 gap-2 flex-grow">
                      {row.map((val, cIdx) => {
                        let cellColor = "bg-slate-900/40 border border-white/5";
                        if (val === 1) cellColor = "bg-teal-900/30 border border-teal-800/20";
                        else if (val === 2) cellColor = "bg-teal-500/30 border border-teal-400/20";
      ...
      {inspectPatientId && (
        <div className="glass-panel p-8 rounded-2xl border border-medical-indigo/35 bg-medical-indigo/5 space-y-6 animate-fade">
          <div className="flex justify-between items-center border-b border-slate-200 dark:border-white/5 pb-3">
            ...
            <button onClick={() => setInspectPatientId(null)} className="text-sm text-slate-400 hover:text-white flex items-center gap-1 font-bold">
          ...
          {inspectError ? (
            <div className="p-4 rounded-xl border border-medical-rose/20 bg-medical-rose/10 text-medical-rose text-sm flex items-start gap-2 leading-relaxed">
              ...
              <div>
                ...
                <p className="mt-2 text-xs uppercase font-bold text-slate-400">Security event logged in admin compliance ledger.</p>
              </div>
            </div>
          ) : patientRecords.length === 0 ? (
            <p className="text-slate-500 py-8 text-center text-base">Checking database ledger records...</p>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Decrypted vitals ledger */}
              <div className="space-y-4">
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Symmetric Decrypted Records</h4>
                <div className="space-y-4 max-h-[300px] overflow-y-auto pr-1">
                  {patientRecords.map(rec => (
                    <div key={rec.id} className="bg-slate-950/60 border border-white/5 p-4 rounded-xl space-y-2 animate-fade">
                      <div className="flex justify-between items-center text-[10px]">
                        ...
                        <span className="text-slate-500">{new Date(rec.updated_at).toLocaleDateString()}</span>
                      </div>
                      <p className="font-mono text-slate-350 bg-black/40 p-2.5 rounded border border-white/5 leading-relaxed text-xs">
              ...
              <div className="space-y-4">
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Vision Analysed Legacy Scans</h4>
                <div className="space-y-4 max-h-[300px] overflow-y-auto pr-1">
                  {patientLegacyImages.length === 0 ? (
                    <p className="text-slate-500 text-xs py-10 text-center">No legacy scans uploaded for this patient.</p>
                  ) : (
                    patientLegacyImages.map(img => (
                      <div key={img.id} className="bg-slate-950/60 border border-white/5 p-4 rounded-xl space-y-2 animate-fade">
                        <div className="flex justify-between items-center text-[10px]">
                          ...
                          <span className="text-slate-500">{new Date(img.created_at).toLocaleDateString()}</span>
                        </div>
                        <p className="font-mono text-slate-350 bg-black/40 p-2.5 rounded border border-white/5 leading-relaxed text-xs whitespace-pre-wrap">
      ...
      {writingPrescAppt && (
        <div className="glass-panel p-8 rounded-2xl border border-medical-teal/35 bg-medical-teal/5 space-y-6 animate-fade">
          <div className="flex justify-between items-center border-b border-slate-200 dark:border-white/5 pb-3">
            ...
            <button onClick={() => setWritingPrescAppt(null)} className="text-sm text-slate-400 hover:text-white flex items-center gap-1 font-bold">
              <ArrowLeft className="w-5 h-5" /> Close Desk
            </button>
          </div>

          {prescNotif && (
            <div className="p-4 rounded-xl bg-slate-950/85 border border-white/10 text-medical-teal font-bold text-base">
              {prescNotif}
            </div>
          )}
          ...
          <form onSubmit={handleWritePrescription} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block font-bold text-slate-400 mb-1.5 text-sm">{t('symptoms')}</label>
                <textarea required rows="2" placeholder="e.g. Mild cough, headache, high temperature..." value={prescForm.symptoms} onChange={e => setPrescForm({...prescForm, symptoms: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white outline-none focus:border-medical-teal text-base"></textarea>
              </div>
              <div>
                <label className="block font-bold text-slate-400 mb-1.5 text-sm">{t('diagnosis')}</label>
                <textarea required rows="2" placeholder="e.g. Seasonal flu, mild throat infection..." value={prescForm.diagnosis} onChange={e => setPrescForm({...prescForm, diagnosis: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white outline-none focus:border-medical-teal text-base"></textarea>
              </div>
            </div>

            <div className="space-y-4 bg-slate-950/20 p-5 border border-white/5 rounded-2xl">
              <label className="block font-bold text-slate-300 text-sm uppercase tracking-wider">Medications List (Rx)</label>
              <div className="space-y-3">
                {medications.map((med, index) => (
                  <div key={index} className="flex flex-col md:flex-row items-center gap-3 bg-slate-900/40 p-4 border border-white/5 rounded-xl animate-fade">
                    <div className="flex-grow w-full md:w-auto">
                      <input 
                        ...
                        className="w-full bg-slate-950 border border-white/10 rounded-xl p-2.5 text-white outline-none focus:border-medical-teal text-sm" 
                      />
                    </div>
            ...
            <div>
              <label className="block font-bold text-slate-400 mb-1.5 text-sm">{t('instructions')}</label>
              <textarea required rows="2" placeholder="Take after meals. Drink plenty of warm water." value={prescForm.instructions} onChange={e => setPrescForm({...prescForm, instructions: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-white outline-none focus:border-medical-teal text-base"></textarea>
            </div>

            {/* Signature Confirmation Checkbox */}
            <div className="flex items-center gap-3 bg-slate-950/40 p-4 rounded-xl border border-white/5 max-w-md">
              <input 
                type="checkbox" 
                id="signCheck" 
                checked={signChecked} 
                onChange={() => setSignChecked(!signChecked)}
                className="rounded border-white/15 bg-slate-900 text-medical-teal focus:ring-0 w-5 h-5" 
              />
              <label htmlFor="signCheck" className="text-xs text-slate-400 cursor-pointer font-bold select-none">
                Apply BMDC Digital Cryptographic Signature
              </label>
            </div>

            <button type="submit" className="bg-medical-teal hover:bg-teal-600 text-white dark:text-medical-darkBg font-extrabold px-6 py-3 rounded-xl flex items-center gap-2 transition-all shadow-md shadow-medical-teal/10 text-base">
      ...
      <div className="glass-panel p-8 rounded-2xl border border-white/5 space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          <h3 className="text-xl font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider">{t('dailyQueue')}</h3>
          ...
          <div className="relative w-full md:w-64">
            <input 
              ...
              className="w-full bg-slate-900 border border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-white placeholder-slate-500 outline-none focus:border-medical-teal text-base"
            />
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-base text-slate-650 dark:text-slate-400">
            <thead>
              <tr className="border-b border-slate-200 dark:border-white/10 text-slate-800 dark:text-slate-300 font-bold">
            ...
            <tbody>
              {filteredQueue.length === 0 ? (
                <tr>
                  <td colSpan="6" className="text-center py-8 text-slate-500">No scheduled appts on queue database ledger.</td>
                </tr>
              ) : (
                filteredQueue.map(appt => (
                  <tr key={appt.id} className="border-b border-slate-100 dark:border-white/5 hover:bg-white/2">
                    <td className="py-4 px-4 font-bold text-slate-800 dark:text-slate-200">{appt.patient_details.username}</td>
                    <td>{appt.date}</td>
                    <td>{appt.time}</td>
                      ...
                      <div className="flex gap-2.5">
                        ...
                        {appt.status === 'approved' && (
                          <>
                            ...
                            <button onClick={() => setWritingPrescAppt(appt)} className="bg-slate-800 hover:bg-slate-700 text-slate-200 border border-white/5 px-4 py-2 rounded-lg text-sm font-bold transition-all">
                              {t('writePrescription')}
                            </button>
                            <button onClick={() => { onApptAction(appt.id, 'complete'); triggerNotification("Session Marked Complete", `Completed consultation with ${appt.patient_details.username}.`, "appointment"); }} className="bg-slate-950 text-slate-400 hover:text-white border border-white/10 px-3 py-1.5 rounded-lg text-xs font-bold transition-all">
                              Mark Completed
                            </button>
                          </>
                        )}
