  ...
  const handleVerifyEmail = () => {
    ...
    setTimeout(() => {
      saveSecState({
        ...
        emailVerified: true,
        alerts: [
          { id: Date.now(), title: 'Email Address Verified', message: 'Direct validation from SwasthoNirapod Security Center.', ip: '127.0.0.1', timestamp: new Date().toLocaleString() },
          ...secState.alerts
        ]
      });
    ...
    <div className="space-y-6 text-xs animate-fade">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass-panel p-6 rounded-2xl flex flex-col justify-between space-y-4">
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Email Verification</span>
        ...
        <div className="glass-panel p-6 rounded-2xl flex flex-col justify-between space-y-4">
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Mobile OTP Verification</span>
            ...
            <form onSubmit={handleVerifyPhone} className="space-y-2.5 w-full">
              <div className="flex gap-2">
                <input 
                  ...
                  className="w-1/2 bg-slate-900 border border-white/10 rounded-lg p-1 text-center font-mono text-white text-xs"
                />
                <button type="submit" className="w-1/2 bg-medical-teal text-medical-darkBg font-bold py-1 rounded-lg">Confirm</button>
              </div>
              <p className="text-[9px] text-slate-500">Code expires in {otpTimer}s. Enter OTP code 123456.</p>
            </form>
          ) : (
            <div className="flex justify-between items-center pt-2">
        ...
        <div className="glass-panel p-6 rounded-2xl flex flex-col justify-between space-y-4">
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">KYC Identity Level</span>
          ...
          <div className="flex justify-between items-center pt-2">
            ...
            <span className="text-[9px] text-slate-500 font-semibold font-mono">Secure Node: #SN-9483</span>
          </div>
        </div>
      ...
      <div className="glass-panel p-6 rounded-2xl space-y-4">
        <div className="flex justify-between items-center border-b border-white/5 pb-2">
          <div>
            ...
            <p className="text-[9px] text-slate-500 mt-0.5">Protect user profile from session hijack attempts</p>
          </div>
          {secState.mfaEnabled && (
            <button 
        ...
        {setupStep === null ? (
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 py-2">
            <div>
              ...
              <p className="text-[10px] text-slate-500 mt-1">
                Multi-factor validation requires entering a secure 6-digit verification credential every time you log in to SwasthoNirapod.
              </p>
            </div>
            {!secState.mfaEnabled && (
              <button 
          ...
          <div className="space-y-4 animate-fade py-2">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <button 
                ...
                className="p-4 rounded-xl border border-white/10 hover:border-medical-teal/30 bg-slate-900/40 text-center transition-all"
              >
                <Smartphone className="w-6 h-6 mx-auto text-slate-400 mb-1" />
                <span className="font-bold block text-slate-200">SMS OTP Code</span>
                <span className="text-[8px] text-slate-500 mt-0.5">Cellular routing via Twilio SMS trunk</span>
              </button>
              ...
              <button 
                ...
                className="p-4 rounded-xl border border-white/10 hover:border-medical-teal/30 bg-slate-900/40 text-center transition-all"
              >
                <Mail className="w-6 h-6 mx-auto text-slate-400 mb-1" />
                <span className="font-bold block text-slate-200">Email OTP Code</span>
                <span className="text-[8px] text-slate-500 mt-0.5">Secure fallback code sent to Gmail inbox</span>
              </button>
              ...
              >
                ...
                <span className="text-[8px] text-slate-500 mt-0.5">Google Authenticator or Microsoft Authenticator</span>
              </button>

            </div>
            <button onClick={() => setSetupStep(null)} className="text-[10px] text-slate-400 hover:text-white underline">Cancel Setup</button>
          </div>
        ) : setupStep === 'totp' ? (
          <form onSubmit={handleMfaSubmit} className="space-y-4 animate-fade py-2 max-w-md">
            ...
            <div className="flex gap-4 items-start bg-slate-950/40 p-4 border border-white/5 rounded-xl">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <input 
                    ...
                    className="bg-slate-900 border border-white/10 rounded-xl px-3 py-1.5 text-white font-mono text-center outline-none focus:border-medical-teal w-36"
                  />
            ...
            <button type="button" onClick={() => setSetupStep('select')} className="text-[10px] text-slate-400 hover:text-white underline">Back</button>
          </form>
          ...
          <form onSubmit={handleMfaSubmit} className="space-y-4 animate-fade py-2 max-w-sm">
            ...
            {otpSent && <p className="text-[9px] text-slate-500">Wait {otpTimer}s to request code resend.</p>}
            <button type="button" onClick={() => setSetupStep('select')} className="text-[10px] text-slate-400 hover:text-white underline block">Back</button>
          </form>
        ) : (
          ...
          <div className="space-y-4 animate-fade py-2">
            ...
            <div className="grid grid-cols-4 gap-2 bg-slate-950/60 p-4 border border-white/5 rounded-xl font-mono text-center font-bold text-slate-200">
              {secState.recoveryCodes.map((code, index) => (
                <div key={index} className="bg-slate-900 border border-white/5 py-1.5 px-2 rounded-lg text-[10px]">
                  {code}
                </div>
              ))}
            </div>
      ...
      <div className="glass-panel p-6 rounded-2xl space-y-4">
        <div className="flex justify-between items-center border-b border-white/5 pb-2">
          <div>
            ...
            <p className="text-[9px] text-slate-500 mt-0.5">Audits real-time hardware credentials and access parameters</p>
          </div>
          ...
          <table className="w-full text-left text-[11px] text-slate-400">
            ...
            <tbody>
              {secState.activeSessions.map((session) => (
                <tr key={session.id} className="border-b border-white/5 hover:bg-white/2">
                  ...
                  <td className="font-mono text-slate-500">{session.loginTime}</td>
                  <td className="text-medical-teal font-semibold">{session.lastActive}</td>
                  <td className="text-right py-2">
                    {session.ip !== '103.145.152.12' ? (
                      <button 
                        onClick={() => terminateSession(session.id)}
                        className="text-slate-500 hover:text-medical-rose p-1.5 transition-colors"
                        title="Force disconnect"
                      >
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                    ) : (
                      <span className="text-[8px] text-slate-500 font-semibold uppercase tracking-wider mr-2">Protected</span>
                    )}
                  </td>
                </tr>
              ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 glass-panel p-6 rounded-2xl space-y-4">
          ...
          <div className="space-y-3 max-h-[200px] overflow-y-auto pr-1">
            {secState.alerts.map((alert) => (
              <div key={alert.id} className="bg-slate-950/40 p-3 rounded-xl border border-white/5 flex justify-between items-start gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-[8px] text-slate-500 font-mono">IP: {alert.ip}</p>
                </div>
                <span className="text-[9px] text-slate-500 font-mono shrink-0">{alert.timestamp}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="glass-panel p-6 rounded-2xl space-y-4">
          <h3 className="text-xs font-bold text-white uppercase tracking-wider border-b border-white/5 pb-2">Change Password</h3>
          ...
          <form onSubmit={handlePasswordSubmit} className="space-y-3">
            <div>
              <label className="block text-slate-400 mb-0.5">Current Password</label>
              <input 
                required
                type="password"
                value={passwordForm.current}
                onChange={e => setPasswordForm({ ...passwordForm, current: e.target.value })}
                className="w-full bg-slate-900 border border-white/10 rounded-xl p-2.5 text-white outline-none focus:border-medical-teal"
              />
            </div>
            <div>
            ...
            <div>
              <label className="block text-slate-400 mb-0.5">Confirm New Password</label>
              <input 
                required
                type="password"
